...
(otro ? 'Otro: ' + otro2 : '');
  ...
  const envioF="H.E. N°"+envio+"-2024-R-UNE";

  
// Luego, inserta derivadoA en la base de datos

  const sql = 'INSERT INTO hojas_rectorado_2024 (n_exp, c, s, dependencia,documentoR, mes, fechaR, asunto, fechaD, plazos, dr, obs, derivadoa, cc1, documentoD,fechaR2, respuesta, cc2, documento2, respuesta3, cc3, documento3,respuesta4, documentoF) VALUES (?, ?, ?, ?, ?, ?, ?, ?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)';
...
app.get('/datos-guardados', (req, res) => {
    const sql = 'SELECT * FROM hojas_rectorado_2024'; // Reemplaza 'h_envio' con el nombre de tu tabla
...
app.delete('/datos-guardados/:id', (req, res) => {
  ...
  const sql = 'DELETE FROM hojas_rectorado_2024 WHERE id = ?';
...
app.put('/datos-guardados', (req, res) => {
  ...
  updatedData.forEach((datum) => {
    ...
    const sql = 'UPDATE hojas_rectorado_2024 SET n_exp = ?, c = ?, s = ?, dependencia = ?, documentoR = ?, mes = ?, fechaR=?, asunto = ?,fechaD=?,plazos=?,dr=?, obs = ?,derivadoa=?, cc1=?,documentoD=?,respuesta=?, cc2=?, documento2=?, respuesta3=?, cc3=?, documento3=?, respuesta4=?, documentoF=? WHERE id = ?';
    db.query(
      sql,
...
app.post('/guardar-datos2', (req, res) => {
  ...
  const sql = 'INSERT INTO hojas_rectorado_2024 (tramite, fechaD, folios, documento, remitido, asunto, observaciones, cc1, cc2, cc3) VALUES (?, ?, ?, ?, ?, ?, ?, ?,?,?)';
...
updatedData.forEach((datum) => {
  ...
  const sql = 'UPDATE hojas_rectorado_2024 SET tramite = ?, fechaD = ?, folios = ?, documento = ?, remitido = ?, asunto = ?, observaciones = ?, cc1 = ?, cc2 =?, cc3=? WHERE id = ?';
  db.query(
    sql,
...
app.post('/guardar-datos3', (req, res) => {
  ...
  const envioF2="H.T. N°"+envio2+"-2024-R-UNE";
   const sql = 'INSERT INTO hojas_rectorado_2024 (n_exp, c, s, dependencia,documentoR, mes, fechaR, asunto, fechaD, plazos, dr, obs, derivadoa, cc1, documentoD,fechaR2, respuesta, cc2, documento2, respuesta3, cc3, documento3,respuesta4, documentoF)VALUES (?, ?, ?, ?, ?, ?, ?, ?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)';
